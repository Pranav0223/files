...
Action descriptions must describe ONE person's body motion only 
from their own body perspective.
Never describe what they do to another person or object.
Focus only on the body movement itself.
Examples:
  "washes feet" → "a person kneels and bends forward"
  "hands over sword" → "a person extends arm forward"
  "places crown on head" → "a person raises both hands upward"
  "pushes someone" → "a person thrusts arms forward"

Position values must be in meters relative to book surface center.
Book surface is approximately 0.2m x 0.28m.
Characters should be within [-0.15, 0.15] on x-axis.
...
def extract_beats(
    ...
    characters = extract_characters_simple(raw_text)
